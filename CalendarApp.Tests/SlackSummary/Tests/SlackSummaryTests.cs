...
namespace CalendarApp.Tests.SlackSummary.Tests {
    public class SlackSummaryTests {
        ...
        public static ICalendarSummaryProvider CalendarSummaryMock (DateTime date) {
            var summaryMock = new Mock<ICalendarSummaryProvider> ();
            summaryMock.Setup (x => x.GetSummary ()).Returns (Summary (date));
            return summaryMock.Object;
        }

        private static CalendarSummary Summary (DateTime date) {
            var calSummaryItems = SummaryItems ();
            var summary = new CalendarSummary (calSummaryItems, DateMock (date));
            return summary;
        }
        ...
        public void Creates_Correct_Json () {
            ...
            Assert.Equal (expectedChunk, actual);
        }
